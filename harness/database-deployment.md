...
운영자는 Web Console Overview, `/ready`, 또는 `GET /api/v1/operations/database-readiness`에서 현재 DB backend, DB URL source, protected PostgreSQL env file 설정 여부, migration version, cutover mode, require-postgres preflight 요약, split credential 준비 여부, 차단 사유를 확인할 수 있다.
...
`runtime_auto_migrate`는 `api`, `worker`별 runtime migration enabled 여부와 값을 결정한 env source만 표시한다.
`database_env_file`은 `SCA_MONITOR_DATABASE_ENV_FILE` 설정 여부와 source 이름만 표시하며 파일 경로, DB URL 원문, host, username, password는 표시하지 않는다.
...
placeholder template 생성 직후 차단 상태를 Web Console/API 증적으로 남기려면 expected-blocked report를 함께 생성한다.

```bash
SCA_MONITOR_PREPARE_DATABASE_ENV_FILE=/data/psyche/Projects/sca-monitor/.secrets/postgres.env \
SCA_MONITOR_PREPARE_DATABASE_ENV_REPORT=expected-blocked \
...
```bash
SCA_MONITOR_CUTOVER_READINESS_REPORT=required \
SCA_MONITOR_CUTOVER_READINESS_REPORT_PATH=.data/cutover-readiness-report.json \
SCA_MONITOR_BACKUP_BEFORE_MIGRATION=required \
SCA_MONITOR_VERIFY_BACKUP_RESTORE=required \
scripts/deploy_remote.sh
