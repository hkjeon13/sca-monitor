...
이 gate는 원격 VM의 `http://127.0.0.1:$SCA_MONITOR_PORT`에 대해 `/health`, `/ready`, `/api/v1/overview`, `/api/v1/operations/cutover-readiness-report`, `/api/v1/operations/worker-leases`, `/`를 확인하고, 설정된 advisory/database backend 기대값도 함께 검증한다.
public tunnel 또는 edge에서 일시 timeout이 관측되는 환경은 `SCA_MONITOR_HTTP_SMOKE_ATTEMPTS=3`과 `SCA_MONITOR_HTTP_SMOKE_RETRY_DELAY_SECONDS=2`를 사용해 endpoint별 재시도를 stop gate에 포함한다.
수동 public smoke도 같은 옵션을 사용한다: `python3 scripts/http_smoke.py --base-url "$SCA_MONITOR_PUBLIC_URL" --attempts 3 --retry-delay-seconds 2 --json`.
...
GitHub malware advisory까지 FR-016 malicious package risk 산정 경로에 반영하려면 별도 bootstrap을 명시한다.
