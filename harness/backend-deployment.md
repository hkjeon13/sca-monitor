...
live dispatcher를 포함하는 `enable` 전환 전에는 `python3 scripts/alert_dispatcher_go_live_gate.py --json`으로 default webhook channel, placeholder target, dry-run dispatcher, dead-letter blocking condition, systemd unit 상태를 확인한다.
