...
- 완료: Daily Digest는 중앙 전체 scope와 `owner_team`별 scope로 Medium 이하 또는 비운영 active impact를 집계하고, scope별 suppression key로 하루 1회 중복 생성을 억제한다. Alert channel은 `owner_team` routing metadata를 저장/조회할 수 있다. 실제 dispatcher team-channel 선택과 운영 발송 정책은 후속 결정 필요
