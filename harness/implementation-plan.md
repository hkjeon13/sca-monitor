...
- 부분 완료: role-aware API 인가는 `SCA_MONITOR_AUTH_MODE=header` impact workflow와 admin-only service registration, endpoint test, push credential, alert channel 설정 범위에서 동작한다. `SCA_MONITOR_AUTH_PROXY_SHARED_SECRET`이 설정되면 인증 프록시가 주입한 `X-SCA-Proxy-Secret`이 일치할 때만 header principal/roles를 신뢰한다. Web Console은 `GET /api/v1/session` capability 기반으로 역할별 action을 비활성화하며, `viewer`는 read-only console capability만 가진다. OIDC/JWT 검증은 미구현
