...
- 완료: push API hardening은 service-scoped `POST /api/v1/services/{service_id}/status` 기본 경로, 기존 `/api/v1/snapshots` 호환 경로, service/environment credential binding, path/body service_id mismatch 거절, `schema_version=1.0`/`environment`/`generated_at`/dependency 필수 필드 검증, payload size limit, dependency count limit, snapshot_id/content_hash 기반 멱등 replay, conflict 감지, `last_confirmed_at` 갱신, service credential 또는 service/environment 기준 분당 rate limit을 제공한다
