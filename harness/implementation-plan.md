...
- 완료: shell/navigation, overview dashboard, service list, basic service registration, push credential issue, integration guide, impact list, impact server-side filters, impact pagination/sorting, impact detail, impact status action panel
- 부분 완료: service registration wizard는 기본 등록과 push credential 발급 form을 제공하며 endpoint test와 credential rotation/revocation UI는 미구현
