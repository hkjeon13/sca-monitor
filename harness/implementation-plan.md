...
- 완료: shell/navigation, overview dashboard, service list/detail, basic service registration, endpoint test action, endpoint one-shot polling worker, push credential issue/rotate/revoke, integration guide, impact list, impact server-side filters, impact 고급 필터 UI, impact pagination/sorting, impact detail, impact status action panel, impact bulk action, header-auth impact/admin API 인가, Web Console role-aware UI 제어, alert channel settings, alert event operations, audit log, advisory detail, accepted risk 승인/만료 workflow, database/canonicalization readiness overview, admin canonicalization apply action
- 부분 완료: service registration wizard는 기본 등록, endpoint test, endpoint bearer auth 설정, polling schedule metadata 입력, push credential 발급/회전/폐기 form을 제공하며 외부 polling scheduler 배치 enable과 조직별 credential rotation 주기 정책은 미구현
...
- 완료: Daily Digest는 중앙 전체 scope와 `owner_team`별 scope로 Medium 이하 또는 비운영 active impact를 집계하고, scope별 suppression key로 하루 1회 중복 생성을 억제한다. Alert channel은 `owner_team` routing metadata를 저장/조회할 수 있고, dispatcher는 Daily Digest team scope를 해당 팀 channel로 우선 발송한 뒤 없으면 default channel로 fallback한다. 실제 팀별 channel 운영 목록과 운영 발송 정책은 후속 결정 필요
...
- 부분 완료: role-aware API 인가는 `SCA_MONITOR_AUTH_MODE=header` impact workflow와 admin-only service registration, endpoint test, push credential, alert channel 설정 범위에서 동작한다. `service-owner`는 자기 팀 단일 impact를 `acknowledged`, `in_progress`, `fixed`, `not_affected`로 전환할 수 있으며, bulk fixed/not_affected는 `admin` 또는 `security-approver`만 수행한다. `SCA_MONITOR_AUTH_PROXY_SHARED_SECRET`이 설정되면 인증 프록시가 주입한 `X-SCA-Proxy-Secret`이 일치할 때만 header principal/roles를 신뢰한다. Web Console은 `GET /api/v1/session` capability와 선택된 impact owner team을 기준으로 action 및 status option을 비활성화하며, `viewer`는 read-only console capability만 가진다. OIDC/JWT 검증은 미구현
