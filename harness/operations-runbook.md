...
SCA_MONITOR_PUBLIC_URL=https://monitoring.fin-ally.net python3 scripts/http_smoke.py --expect-cutover-report-status ok --json
SCA_MONITOR_PUBLIC_URL=https://monitoring.fin-ally.net python3 scripts/http_smoke.py --expect-cutover-report-status ok --expect-cutover-report-production-preflight-status ok --json
...
NVD_REQUEST_DELAY_SECONDS=1 python3 scripts/nvd_cve_sync.py --use-cursor --lookback-hours 24 --modified-results-per-page 2000 --limit 100
```

`--delay-seconds` 또는 `NVD_REQUEST_DELAY_SECONDS`로 원격 NVD API batch 요청 간격을 조절한다.
`--last-mod-start`/`--last-mod-end`는 NVD modified window에서 CVE ID 후보를 먼저 찾은 뒤 기존 batch import 경로를 사용한다. 원격 후보 조회는 `startIndex`/`resultsPerPage` pagination으로 전체 window를 순회하며, `--modified-results-per-page`로 page size를 조절한다.
...
modified-window 후보 CVE가 0개인 경우도 정상 동기화로 간주하며 `status=ok`, `records_processed=0`, cursor=window end를 저장한다.
...
```bash
SCA_MONITOR_DEFAULT_ALERT_WEBHOOK_URL=https://alert-router.example/webhook \
