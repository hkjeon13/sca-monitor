...
| REQ-AUTH-005 | endpoint polling 인증 | PARTIAL | endpoint test API/UI, schema 검증, polling worker, DB lease 기반 중복 실행 방지는 구현됨. bearer token, mTLS, HMAC 중 운영 인증 범위와 secret 저장 방식 결정 필요 |
