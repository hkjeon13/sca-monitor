...
| REQ-AUTH-002 | 역할 모델 | PARTIAL | `admin`, `service-owner`, `security-approver`, `viewer`. `SCA_MONITOR_AUTH_MODE=header`에서는 `X-SCA-Principal`, `X-SCA-Roles`, `X-SCA-Owner-Teams`를 인증 프록시가 주입해야 하며, `SCA_MONITOR_AUTH_PROXY_SHARED_SECRET`이 설정된 경우 `X-SCA-Proxy-Secret` 일치도 요구한다. Web Console은 `/api/v1/session` capability로 action을 제어하고, `viewer`는 `view_console=true` 및 변경성 capability false로 표시된다. `service-owner`는 자기 팀 단일 impact를 `acknowledged`, `in_progress`, `fixed`, `not_affected`로 전환할 수 있으며, bulk fixed/not_affected는 `admin` 또는 `security-approver`만 수행한다 |
...
| ID | 항목 | 상태 | 필요 결정 |
|---|---|---|---|
| REQ-ALERT-001 | Slack workspace | PARTIAL | Slack incoming webhook channel type 등록/API/UI/test/dispatcher payload 변환은 구현됨. Slack app 방식은 후속 결정 필요 |
| REQ-ALERT-002 | webhook 대상 | PARTIAL | 기본 webhook/Slack incoming webhook channel 등록과 dispatcher 기본 target 사용은 구현됨. 운영 alert router/Jira/Teams 실제 URL과 secret 보관 방식 결정 필요 |
| REQ-ALERT-003 | Daily Digest 채널 | PARTIAL | 중앙 digest와 `owner_team`별 digest outbox/preview scope, alert channel의 `owner_team` routing metadata 저장/조회, dispatcher의 Daily Digest team channel 우선 선택과 default fallback, Slack incoming webhook digest payload 변환은 구현됨. 실제 팀별 channel 운영 목록 결정 필요 |
