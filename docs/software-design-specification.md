...
- SLA Evaluation: active impact(`open`, `acknowledged`, `in_progress`)는 risk level별 기본 SLA(critical 24h, high 72h, medium 7d, low/info 30d)를 기준으로 API 응답의 `sla.deadline_at`, `sla.overdue`, `sla.seconds_until_deadline`을 계산한다. Overview와 `/metrics`는 `sla_overdue_impacts`/`sca_monitor_sla_overdue_impacts`를 노출한다
...
- Alert Channel Settings: `GET/POST/PATCH /api/v1/settings/alert-channels`와 Web Console Settings에서 기본 webhook channel을 등록, 조회, default 전환, disable 처리한다. `POST /api/v1/settings/alert-channels/{channel_id}/test`와 Web Console `Test` action은 alert outbox row를 claim/send 처리하지 않고 synthetic payload로 channel 연결성을 검증한다. webhook URL 원문은 조회 응답에 노출하지 않는다
