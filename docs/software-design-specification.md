...
- Alert Channel Settings: `GET/POST/PATCH /api/v1/settings/alert-channels`와 Web Console Settings에서 기본 webhook channel을 등록, 조회, default 전환, disable 처리한다. 채널 목록은 placeholder target 여부와 live dispatcher 차단 상태를 표시한다. `POST /api/v1/settings/alert-channels/{channel_id}/test`와 Web Console `Test` action은 alert outbox row를 claim/send 처리하지 않고 synthetic payload로 channel 연결성을 검증한다. webhook URL 원문은 조회 응답에 노출하지 않는다
