...
- Operational Metrics: `/metrics`에서 service/open impact/critical/high/unhealthy count와 advisory sync ready/initialized/lag, endpoint poll success rate, alert delivery success rate, alert outbox pending/dead-letter count, stale service count를 Prometheus text 형태로 노출
