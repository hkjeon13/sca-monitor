...
- Database Readiness Console: `GET /api/v1/operations/database-readiness`와 Web Console Overview는 `/ready`의 DB/migration 상태, credential을 제외한 DB URL source, 현재 PostgreSQL cutover 판정, require-postgres 전환 차단 사유를 함께 표시해 FR-030의 운영 모니터링 범위를 DB 전환 상태까지 확장한다
