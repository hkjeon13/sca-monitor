...
- Push Credential: `POST /api/v1/services/{service_id}/push-credentials` 기반 `snapshot:push` token 발급, token hash 저장, service/environment 바인딩 검증. `POST /api/v1/services/{service_id}/push-credentials/{credential_id}/revoke` 기반 revoke와 `POST /api/v1/services/{service_id}/push-credentials/{credential_id}/rotate` 기반 회전을 지원한다. Web Console에서 token을 1회 표시하고 발급/회전 직후 해당 service/environment/token을 반영한 `POST /api/v1/services/{service_id}/status` curl snippet을 제공하며 credential rotate/revoke action을 지원
