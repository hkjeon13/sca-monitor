...
GHSA_BOOTSTRAP="${SCA_MONITOR_GHSA_BOOTSTRAP:-disabled}"
GHSA_BOOTSTRAP_LIMIT="${SCA_MONITOR_GHSA_BOOTSTRAP_LIMIT:-1}"
GHSA_MALWARE_BOOTSTRAP="${SCA_MONITOR_GHSA_MALWARE_BOOTSTRAP:-disabled}"
GHSA_MALWARE_BOOTSTRAP_LIMIT="${SCA_MONITOR_GHSA_MALWARE_BOOTSTRAP_LIMIT:-1}"
CANONICAL_MERGE="${SCA_MONITOR_CANONICAL_MERGE:-disabled}"
CANONICAL_MERGE_LIMIT="${SCA_MONITOR_CANONICAL_MERGE_LIMIT:-100}"
CANONICAL_MERGE_DRY_RUN="${SCA_MONITOR_CANONICAL_MERGE_DRY_RUN:-true}"
...
EXPECT_ADVISORY_SOURCE_STATUS="${SCA_MONITOR_EXPECT_ADVISORY_SOURCE_STATUS:-}"
...
ssh "$REMOTE" "set -euo pipefail
  ...
  GHSA_BOOTSTRAP='$GHSA_BOOTSTRAP'
  GHSA_BOOTSTRAP_LIMIT='$GHSA_BOOTSTRAP_LIMIT'
  GHSA_MALWARE_BOOTSTRAP='$GHSA_MALWARE_BOOTSTRAP'
  GHSA_MALWARE_BOOTSTRAP_LIMIT='$GHSA_MALWARE_BOOTSTRAP_LIMIT'
  CANONICAL_MERGE='$CANONICAL_MERGE'
  CANONICAL_MERGE_LIMIT='$CANONICAL_MERGE_LIMIT'
  CANONICAL_MERGE_DRY_RUN='$CANONICAL_MERGE_DRY_RUN'
  POST_DEPLOY_HTTP_SMOKE='$POST_DEPLOY_HTTP_SMOKE'
  HTTP_SMOKE_ATTEMPTS='$HTTP_SMOKE_ATTEMPTS'
  HTTP_SMOKE_RETRY_DELAY_SECONDS='$HTTP_SMOKE_RETRY_DELAY_SECONDS'
  EXPECT_POSTGRES_SPLIT_REQUIRED='$EXPECT_POSTGRES_SPLIT_REQUIRED'
  EXPECT_ADVISORY_SYNC_READY='$EXPECT_ADVISORY_SYNC_READY'
  EXPECT_ADVISORY_SOURCE_STATUS='$EXPECT_ADVISORY_SOURCE_STATUS'
  EXPECT_DATABASE_BACKEND='$EXPECT_DATABASE_BACKEND'
  EXPECT_DATABASE_ENV_FILE_CONFIGURED='$EXPECT_DATABASE_ENV_FILE_CONFIGURED'
  EXPECT_CUTOVER_REPORT_STATUS='$EXPECT_CUTOVER_REPORT_STATUS'
  EXPECT_CUTOVER_REPORT_EXPECTED_STATUS='$EXPECT_CUTOVER_REPORT_EXPECTED_STATUS'
  EXPECT_CUTOVER_REPORT_PRODUCTION_PREFLIGHT_STATUS='$EXPECT_CUTOVER_REPORT_PRODUCTION_PREFLIGHT_STATUS'
  REQUIRE_CUTOVER_REPORT_EXPECTATION_MET='$REQUIRE_CUTOVER_REPORT_EXPECTATION_MET'
  ...
  if [ -n \"\$SYSTEMD_REQUIRE_ACTIVE_UNITS_OVERRIDE\" ]; then
    SCA_MONITOR_SYSTEMD_REQUIRE_ACTIVE_UNITS=\"\$SYSTEMD_REQUIRE_ACTIVE_UNITS_OVERRIDE\"
  fi
  ...
  export SCA_MONITOR_SYSTEMD_REQUIRE_ACTIVE_UNITS=\"\${SCA_MONITOR_SYSTEMD_REQUIRE_ACTIVE_UNITS:-}\"
...
PY
  ...
  case \"\$GHSA_BOOTSTRAP\" in
    disabled|skip|false|0|'')
      echo 'GHSA bootstrap skipped'
      ;;
    required|sync|true|1|yes|on)
      python3 scripts/ghsa_sync.py --limit \"\$GHSA_BOOTSTRAP_LIMIT\" --lock-owner deploy-ghsa-bootstrap --lock-ttl-seconds 3600
      ;;
    *)
      echo \"invalid SCA_MONITOR_GHSA_BOOTSTRAP: \$GHSA_BOOTSTRAP\" >&2
      exit 2
      ;;
  esac
  case \"\$GHSA_MALWARE_BOOTSTRAP\" in
    disabled|skip|false|0|'')
      echo 'GHSA malware bootstrap skipped'
      ;;
    required|sync|true|1|yes|on)
      python3 scripts/ghsa_sync.py --type malware --limit \"\$GHSA_MALWARE_BOOTSTRAP_LIMIT\" --lock-owner deploy-ghsa-malware-bootstrap --lock-ttl-seconds 3600
      ;;
    *)
      echo \"invalid SCA_MONITOR_GHSA_MALWARE_BOOTSTRAP: \$GHSA_MALWARE_BOOTSTRAP\" >&2
      exit 2
      ;;
  esac
  case \"\$CANONICAL_MERGE\" in
    disabled|skip|false|0|'')
      echo 'canonical advisory merge skipped'
      ;;
    required|run|true|1|yes|on)
      case \"\$CANONICAL_MERGE_DRY_RUN\" in
        true|1|yes|on|'')
          python3 scripts/merge_canonical_advisories.py --limit \"\$CANONICAL_MERGE_LIMIT\" --actor deploy-canonical-merge --dry-run
          ;;
        false|0|no|off)
          python3 scripts/merge_canonical_advisories.py --limit \"\$CANONICAL_MERGE_LIMIT\" --actor deploy-canonical-merge
          ;;
        *)
          echo \"invalid SCA_MONITOR_CANONICAL_MERGE_DRY_RUN: \$CANONICAL_MERGE_DRY_RUN\" >&2
          exit 2
          ;;
      esac
      ;;
    *)
      echo \"invalid SCA_MONITOR_CANONICAL_MERGE: \$CANONICAL_MERGE\" >&2
      exit 2
      ;;
  esac
  ...
  if [ \"\$api_ready\" = '1' ]; then
    case \"\$POST_DEPLOY_HTTP_SMOKE\" in
      disabled|skip|false|0|'')
        ...
        ;;
      auto|required)
        http_smoke_args=(--base-url http://127.0.0.1:$PORT --attempts \"\$HTTP_SMOKE_ATTEMPTS\" --retry-delay-seconds \"\$HTTP_SMOKE_RETRY_DELAY_SECONDS\")
        ...
        if [ -n \"\$EXPECT_ADVISORY_SOURCE_STATUS\" ]; then
          advisory_source_status_items=\"\${EXPECT_ADVISORY_SOURCE_STATUS//,/ }\"
          for advisory_source_status in \$advisory_source_status_items; do
            http_smoke_args+=(--expect-advisory-source-status \"\$advisory_source_status\")
          done
        fi
